    ...
    S: Service,
{
    ...
    match &action.action {
        ...
        Action::PrecheckerPrecheckOperationInit(PrecheckerPrecheckOperationInitAction {
            ...
            match prechecker_state_operations.get(key).map(|op| &op.state) {
                Some(PrecheckerOperationState::Init { .. }) => {
                    store.dispatch(PrecheckerGetProtocolVersionAction {
                        key: key.clone(),
        ...
        Action::PrecheckerGetProtocolVersion(PrecheckerGetProtocolVersionAction {
            key,
            branch,
        }) => {
            if let Some((_, proto)) = store
                .state
                .get()
                .prechecker
                ...
                .get(branch)
            {
                ...
                store.dispatch(PrecheckerProtocolVersionReadyAction {
                    key: key.clone(),
                    proto,
                });
            } else {
                store.dispatch(StorageBlockAdditionalDataGetAction {
                    key: branch.clone().into(),
                });
            }
        }
        Action::StorageBlockAdditionalDataOk(StorageBlockAdditionalDataOkAction { key, value }) => {
            for key in store
                ...
                .collect::<Vec<_>>()
            {
                match value.next_protocol_hash().try_into() {
                    Ok(proto) => {
                        store.dispatch(PrecheckerProtocolVersionReadyAction { key, proto })
                    }
                    Err(err) => store.dispatch(PrecheckerErrorAction {
                        key,
                        error: err.into(),
                    }),
                };
        ...
        Action::PrecheckerProtocolVersionReady(PrecheckerProtocolVersionReadyAction {
            key,
            ..
        }) => {
        ...
        Action::PrecheckerDecodeOperation(PrecheckerDecodeOperationAction { key }) => {
            if let Some(PrecheckerOperation {
                ...
                state: PrecheckerOperationState::PendingContentDecoding { proto },
                ..
